...
def process(params):
    ...
    while (a == True):
        ...
        # ddf.drop(ddf[ddf.date.values > elp_time].index, inplace = True)
        #ddf = ddf[(ddf['date'].values > elp_time)]
        ...
        try:
            ...
            # if not ddf_filtered_for_price_change.empty:
            #     max_check = (date_fin- date)
            
            # if (max_check.total_seconds()/60 > 90):
            #     time_chk = ddf[(ddf['date'] >= date + + datetime.timedelta(minutes = 90))]
            ...
            # print("elapsed_time:" + str(elapsed_time.total_seconds()/60))
            ...
            trade_data.append(
                ...
                'week_year': str(date.isocalendar()[1]) +"-"+ str(date.today().isocalendar()[0]),
                'act_fall_from_pct': act_pct_from_mvg,
                'act_devs': act_devs,
                'pct_from_min': pct_from_min,
                'pnl':profit,
                'elapsed_time':elapsed_time.total_seconds()/60,
                'avg_pct_chg': Average(all_prices),
                'min_pct_chg': min(all_prices)
                }
            )

            df2 = pd.DataFrame(trade_data)
            # pivoted_trade_data = df2.groupby('month')['pnl'].agg(['sum'])
            df2.to_csv('trades_'+str(t0)+".csv", index=False, mode='w+')
    ...
    ##df.to_csv('in_sample_sept_oct.csv', index=False, mode='w+')
    
    end_check = time.time()
    elapsed_check = end_check - start_check
    print(elapsed_check/60)
    return -1*cumulative_profit
print("yes")
...
#OS 3
# ddf = ddf[(((pd.DatetimeIndex(ddf['date']).month ==3)|(pd.DatetimeIndex(ddf['date']).month ==4)|(pd.DatetimeIndex(ddf['date']).month ==5))&(pd.DatetimeIndex(ddf['date']).year ==2021))]

ddf = ddf.astype({"symbol": 'category'})

# ddf = ddf[(ddf['symbol']!='XRP/USDT')]


# ddf = ddf[(((pd.DatetimeIndex(ddf['date']).month ==12)|(pd.DatetimeIndex(ddf['date']).month ==12))&(pd.DatetimeIndex(ddf['date']).year ==2020))]

# ddf = ddf[(((pd.DatetimeIndex(ddf['date']).month ==9)|(pd.DatetimeIndex(ddf['date']).month ==10)|(pd.DatetimeIndex(ddf['date']).month ==11)|(pd.DatetimeIndex(ddf['date']).month ==12))&(pd.DatetimeIndex(ddf['date']).year ==2020))|
# ((((pd.DatetimeIndex(ddf['date']).month ==1)|(pd.DatetimeIndex(ddf['date']).month ==2))&(pd.DatetimeIndex(ddf['date']).year ==2021)))]
...
#process((-0.09,0.9, -2.1,60,24*14))
#process((-0.05,1, -2,120,24*14))

process((-0.06,0.9, -2.1,60,24*14))
